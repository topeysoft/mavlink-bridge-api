...
export class MAVLinkCommands {
  ...
  private async searchParameters (): Promise<void> {
    ...
    try {
      ...
      if (results.hasMore) {
        ...
        const showMore = await this.uiHelpers.confirmAction(
          'Show additional results with less restrictive search?',
          false
        );
  ...
  private async setParameter (): Promise<void> {
    ...
    if (definition) {
      ...
      if (definition.range) {
        ...
        if (paramValue < definition.range.min || paramValue > definition.range.max) {
          ...
          const proceedAnyway = await this.uiHelpers.confirmAction(
            'Continue with out-of-range value? This could cause unexpected behavior.',
            false
          );
    ...
    if (currentParameter && currentParameter.value === paramValue) {
      ...
      const continueSame = await this.uiHelpers.confirmAction(
        'Continue setting the same value?',
        false
      );
  ...
  private async restoreParameters (): Promise<void> {
    ...
    try {
      ...
      if (backupData.cacheStats?.categoryCounts) {
        ...
        const categoryTable = this.uiHelpers.createTable(['Category', 'Count']);
        Object.entries(backupData.cacheStats.categoryCounts).forEach(([category, count]: [string, unknown]) => {
  ...
  private async refreshParameters (): Promise<void> {
    ...
    const confirmed = await this.uiHelpers.confirmAction(
      'Clear cache and refresh all parameters? This may take several minutes.',
      true
    );
  ...
  private async listMission (): Promise<void> {
    ...
    try {
      ...
      mission.items.slice(0, 20).forEach((item: any, index: number) => {
        table.push([
          (index + 1).toString(),
  ...
  private parseHeartbeat (payload: string): any {
    try {
  ...
  private parseAttitude (payload: string): any {
    try {
      const buffer = Buffer.from(payload, 'base64');
  ...
  private parseGlobalPosition (payload: string): any {
    try {
      const buffer = Buffer.from(payload, 'base64');
      if (buffer.length < 28) return null;
  ...
  private parseGpsRaw (payload: string): any {
    try {
      const buffer = Buffer.from(payload, 'base64');
  ...
  private parseBattery (payload: string): any {
    try {
      const buffer = Buffer.from(payload, 'base64');
  ...
  private parseSysStatus (payload: string): any {
    try {
      const buffer = Buffer.from(payload, 'base64');
  ...
  private parseVfrHud (payload: string): any {
    try {
      const buffer = Buffer.from(payload, 'base64');
  ...
  private getGpsFixType (fixType: number): string {
    switch (fixType) {
