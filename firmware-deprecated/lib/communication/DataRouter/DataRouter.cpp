...
DataRouter::DataRouter() : activeInterface(NONE),
...
void DataRouter::routeUpstream(const uint8_t *data, size_t length)
{
    ...
    ESP_LOGD(TAG, "📤 DataRouter: Routing %d bytes upstream to processing queue", length);
    ESP_LOGD(TAG, "📤 DataRouter: Queuing %d bytes for MAVLink processing", length);
    ...
    {
        ESP_LOGD(TAG, "✅ DataRouter: Packet queued for processing");
    }
}
...
void DataRouter::processRouting()
{
    ...
    {
        ESP_LOGD(TAG, "🔄 DataRouter: Processing upstream packet - %d bytes from %s",
                 packet.length, interfaceToString(packet.source));
        ESP_LOGD(TAG, "🔄 DataRouter: Processing %d bytes from %s",
                 packet.length, interfaceToString(packet.source));

        if (mavlinkProcessingEnabled)
        {
            ESP_LOGD(TAG, "📡 DataRouter: Sending to MAVLink processor");
            ESP_LOGD(TAG, "📡 DataRouter: Sending to MAVLink processor");

            // Log first few bytes for MAVLink debugging (verbose level only)
            ...
            ESP_LOGD(TAG, "✅ DataRouter: MAVLink processing completed");
        }
        else
        {
...
{
    ESP_LOGD(TAG, "🔄 DataRouter: Received USB data - %d bytes", length);
    ESP_LOGD(TAG, "🔄 DataRouter: Received USB data - %d bytes", length);
    ESP_LOGD(TAG, "Active interface: %s", interfaceToString(activeInterface));

    if (activeInterface == USB_OTG)
    {
        ESP_LOGD(TAG, "✅ DataRouter: Routing %d bytes upstream to MAVLink processor", length);
        ESP_LOGD(TAG, "✅ DataRouter: Routing to MAVLink processor");
