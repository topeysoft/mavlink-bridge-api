...
export class RTCMCommands {
    ...
    async startClient() {
        ...
        try {
            ...
            const useCurrentConfig = await this.uiHelpers.confirmAction('Use current configuration to start RTCM client?', true);
            if (!useCurrentConfig) {
    ...
    async configureAndStart(client) {
        console.log(chalk.blue('\n⚙️  Configure RTCM Client'));
        const sourceType = await this.uiHelpers.selectFromList('Select RTCM source type:', [
        ...
        if (sourceType === 'tcp') {
            const host = await this.uiHelpers.getTextInput('Enter TCP host (IP address):', '192.168.1.100');
            const port = await this.uiHelpers.getNumberInput('Enter TCP port:', 2101, 1, 65535);
            sourceConfig = { type: 'tcp', host, port };
        }
        else if (sourceType === 'ntrip') {
            const host = await this.uiHelpers.getTextInput('Enter NTRIP caster host:', 'rtk2go.com');
            const port = await this.uiHelpers.getNumberInput('Enter NTRIP port:', 2101, 1, 65535);
            const mountpoint = await this.uiHelpers.getTextInput('Enter mountpoint:', 'MOUNT');
            const needsAuth = await this.uiHelpers.confirmAction('Does this NTRIP caster require authentication?', false);
            ...
            const port = await this.uiHelpers.getNumberInput('Enter UDP listen port:', 14550, 1, 65535);
    ...
    async stopClient() {
        ...
        const confirmed = await this.uiHelpers.confirmAction('Stop the RTCM client?', false);
        if (!confirmed) {
...
//# sourceMappingURL=RTCMCommands.js.map