...
class CompleteSetupRequest(BaseModel):
    ...
    admin_key_description: str | None = Field(
        default="Owner's admin key created during setup",
        max_length=256,
...
async def complete_setup(request: CompleteSetupRequest) -> CompleteSetupResponse:
    ...
    try:
        ...
        return CompleteSetupResponse(
            ...
            device_name=request.device_name,
            message="Setup completed successfully! Save your API key - it will not be shown again.",
        )
...
async def reset_setup() -> dict[str, str]:
    ...
    logger.warning("setup_reset", note="All API keys deleted, setup mode enabled")

    return {
        "message": "Setup has been reset. Device is now in setup mode.",
        "warning": "All API keys have been deleted.",
    }
