...
void WiFiEndpoints::handleConnect(const HttpRequest &req, HttpResponse &res)
{
    ...
    String ssid = String(body["ssid"].as<const char*>());
    String password = String(body["password"].as<const char*>());
    ...
    // Save network credentials (replaces any existing saved network)
    wifiManager->saveNetwork(ssid, password);
...
void WiFiEndpoints::handleStatus(const HttpRequest &req, HttpResponse &res)
{
    ...
    if (state == WiFiManager::CONNECTED)
    {
        ...
        if (connInfo.rssi >= -50)
            quality = 100;
        else if (connInfo.rssi >= -60)
            quality = 80;
        else if (connInfo.rssi >= -70)
            quality = 60;
        else if (connInfo.rssi >= -80)
            quality = 40;
        else
            quality = 20;
...
void WiFiEndpoints::handleScan(const HttpRequest &req, HttpResponse &res)
{
    ...
    bool forceNew = false;
    AsyncWebServerRequest *request = static_cast<AsyncWebServerRequest *>(req.request);
    if (request->hasParam("force"))
    {
        String forceParam = request->getParam("force")->value();
